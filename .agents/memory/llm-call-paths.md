...
**Default base-URL routing has THREE sites, not two:** besides the bot (toolChat `runToolChat`) and agent completion (llm.ts `callOpenAiCompatible`), the **model-listing** path (llm.ts `listModels`, used by "Fetch models" / Test) also picks a default base URL independently. Per-provider defaults live in the leaf module `providerDefaults.ts` (`defaultOpenAiCompatibleBase(providerType)`) so all three resolve identically — OpenRouter → `https://openrouter.ai/api/v1`, everything else OpenAI-compatible → `https://api.openai.com/v1`. **Why:** OpenRouter's Base URL is optional in the UI; a blank one previously inherited the OpenAI default, so an OpenRouter key + `deepseek/...` model was sent to OpenAI (402/401, surfaced as an opaque 502 on Fetch models). An explicit user Base URL (or host/port) still wins via `resolveBaseUrl`/`resolveBaseFromInput`. Any new provider needing a non-OpenAI default must be added to `providerDefaults.ts`, which all three sites consume.

# Which model the bot uses (unified — single selector)

The Telegram bot and the in-app Chat assistant are the SAME bot agent and resolve their model the SAME way: `resolveAgentModel(tenantId, botAgent.id)` reading the bot agent's row in **`agent_model_policies`**. No policy row ⇒ managed-Anthropic default. There is NO separate Telegram model setting anymore — the old `resolveTelegramEndpoint()` / `tenants.settingsJson.telegramModelEndpointId` / `PUT /telegram/model` were removed; `GET /telegram/model` is read-only and just reports the resolved endpoint name.
**Why:** a prod incident — Telegram and in-app diverged because each pointed at a different endpoint, so switching one to Claude left the other on weak deepseek. The agent is now the single source of truth for both surfaces.
**How to apply:** to change the bot's model for BOTH surfaces, set the ContextOS Bot agent's model policy (agent-detail UI `PUT /agents/:id/model-policy`, or the bot self-configuring via MCP `list_model_endpoints` + `set_agent_model`). Channel-only differences that legitimately remain: `TELEGRAM_CHANNEL_NOTE` (plain-text formatting), 48h Telegram history pruning, and `telegramChatId`-based async run notifications. `secretStore` lives at the api-server cwd: `artifacts/api-server/.local/state/model-secrets.json`.
